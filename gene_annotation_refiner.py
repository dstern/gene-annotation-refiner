...
class ScoringConfig:
    ...
    def __init__(self):
        ...
        self.coding_threshold = 0.50
        self.ncrna_threshold = 0.30
...
class ORFFinder:
    ...
    def find_best_orf(self, seqid: str, exons: List[Feature],
                      strand: str) -> Optional[Tuple[int, int, int]]:
        """Find the longest ORF in a transcript.
        ...
        for frame in range(3):
            orfs = self._find_orfs_in_frame(tx_seq, frame)
            for orf_start, orf_end in orfs:
                orf_len = orf_end - orf_start
                if orf_len > best_len:
                    best_len = orf_len
                    best_orf = (orf_start, orf_end, frame)

        return best_orf

    def _find_orfs_in_frame(self, seq: str, frame: int) -> List[Tuple[int, int]]:
        """Find all ORFs starting with ATG and ending at a stop codon."""
        orfs = []
        seq_len = len(seq)
        i = frame
            ...
            if codon == self.START_CODON:
                ...
                while j <= seq_len - 3:
                    ...
                    if c in self.STOP_CODONS:
                        orfs.append((i, j + 3))  # include stop codon
                        break
                    j += 3
                else:
                    # No stop: partial ORF to end
                    if seq_len - i >= 150:
                        orfs.append((i, seq_len))
                # Skip past this ORF's start to find nested ORFs
                i += 3
            else:
                i += 3
...
class CoverageAccess:
    ...
    def get_mean_coverage(self, seqid: str, start: int, end: int) -> float:
        """Get mean coverage for a region."""
        cov = self.get_coverage(seqid, start, end)
        return np.mean(cov) if len(cov) > 0 else 0.0
...
class GeneAnnotationRefiner:
    ...
    def _build_consensus(self) -> List[Gene]:
            ...
            for start, end, sources in exon_list:
                ...
                for m in merged:
                    if abs(m[0] - start) <= DEDUP_TOL and abs(m[1] - end) <= DEDUP_TOL:
                        m[2].update(sources)
                        # Use StringTie boundaries if available (RNA-seq based)
                        if 'StringTie' in sources and 'StringTie' not in (m[2] - sources):
                            m[0], m[1] = start, end
                        placed = True
                        break
                if not placed:
        ...
        for k, info in candidate_exons.items():
            ...
            # Source count score (0-1): more sources = more confident
        ...
        for k, info in candidate_exons.items():
            ...
            scored_exons_by_region[(seqid, strand)].append(
                (start, end, info['score'], info['sources'], info['coverage']))
        ...
        for source_label, template in all_templates:
            if not template.transcripts:
                continue
            best_tx = max(template.transcripts, key=lambda tx: len(tx.exons))
            ...
            # An internal exon is dropped if:
            # - It has score < 0.15 (very weak evidence)
            # - It's supported by only 1 source
            # - It has < 1.0 RNA-seq coverage
            # - Dropping it would create a gap bridgeable by canonical splice sites
            if len(gene_exons) >= 3:
                ...
                for i in range(1, len(sorted_ge) - 1):
                    ...
                    cov_match = find_matching_exon(seqid, strand, exon.start, exon.end)
                    exon_cov = cov_match[4] if cov_match else 0.0

                    if (exon.score < 0.15 and len(src_set) <= 1 and exon_cov < 1.0):
                        # Check if removal creates a canonical intron
                        ...
                        if gap_e - gap_s + 1 >= MIN_INTRON_SIZE:
                            ...
                            if len(d) >= 2 and len(a) >= 2:
                                if (d == 'GT' and a == 'AG') or (d == 'GC' and a == 'AG'):
                                    ...
                                    logger.debug(f"Dropping weak exon {exon.start}-{exon.end} "
                                               f"(score={exon.score:.2f}, sources={src_set})")
            ...
            # - Are supported by 2+ sources OR have strong coverage
            ...
            if gene_exons:
                ...
                while idx < len(pool_exons):
                    ...
                    # Require strong evidence: 2+ sources OR good coverage + 1 source
                    if len(psrc) < 2 and pcov < 5.0:
                        continue
            ...
            mean_score = total_score / len(gene_exons)
            ...
            # Carry over CDS from template if available
            if best_tx.cds:
        ...
        assembled_genes.sort(key=lambda x: -x[0])
        ...
        for gene in genes:
            ...
            for tx in gene.transcripts:
                ...
                for i, exon in enumerate(sorted(tx.exons, key=lambda e: e.start), 1):
                    exon_attrs = f"ID={tx.transcript_id}.exon.{i};Parent={tx.transcript_id}"
...
--transdecoder, --manual_annotation, or --refine_existing must be provided.
    ...
    parser.add_argument('--ncRNA_threshold', type=float, default=0.30,
                        help='Posterior threshold for ncRNA (default: 0.30)')
    parser.add_argument('--coding_threshold', type=float, default=0.50,
                        help='Posterior threshold for coding genes (default: 0.50)')
    ...
    if args.ncRNA_threshold != 0.30:
        scoring_config.ncrna_threshold = args.ncRNA_threshold
    if args.coding_threshold != 0.50:
